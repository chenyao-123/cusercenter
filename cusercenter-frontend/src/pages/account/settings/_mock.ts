...
function getCity(req: Request, res: Response) {
  // 【修复核心】：显式将 req.params.province 断言为 string
  // 因为 JSON 对象的 key 必须是 string，而 TS 认为 params 可能是 string[]
  const provinceKey = req.params.province as string;

  // 可选：增加一个安全检查，防止 key 不存在导致返回 undefined
  const result = city[provinceKey] || [];
